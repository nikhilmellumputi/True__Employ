...
from libraries.Courses import ds_course,web_course,android_course,ios_course,uiux_course,resume_videos,interview_videos
import pafy
...
word_vectorizer = joblib.load("libraries/Resume_classifier/word_vectorizer.pkl")
model = joblib.load("libraries/Resume_classifier/resume_classification.pkl")
RESUME_KEYS = {6: 'Data Science',
 12: 'HR',
 0: 'Advocate',
 1: 'Arts',
 24: 'Web Designing',
...
def reportgeneration():
    ...
    save_image_path = os.path.join(app.config['UPLOAD_FOLDER'], session['cv'])
    resume_text = pdf_reader(save_image_path)
    to_predict = cleanResume(resume_text)
    to_predict = [to_predict]
    vectorised = word_vectorizer.transform(to_predict)
    probability = model.predict_proba(vectorised)
    top_n = 2
    top_predictions = []
    ...
    insert_class_predictions(top_predictions,session['id'])
    command = "select stud_first_name, stud_last_name, stud_class, science, humanities, commerce, aptitude, total ,pred_class1, pred_class2 from student_profile where stud_id =%s "
    cursor.execute(command, session['id'])
    res = cursor.fetchall()
    cursor.close()
...
def insert_class_predictions(predictions,stud_id):
    predictions = predictions[0]
    class1,class2 = predictions[0],predictions[1]
    class1 = RESUME_KEYS[class1]
    class2 = RESUME_KEYS[class2]
    cursor = connection.cursor()
    DB_table_name = 'student_profile'
    insert_sql = "UPDATE " + DB_table_name + """ SET pred_class1 = '{}' ,pred_class2 = '{}' where stud_id={}"""
    insert_sql = insert_sql.format(class1,class2,stud_id)
    print(insert_sql)
